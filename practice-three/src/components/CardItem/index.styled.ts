...
const CardItemStyled = styled.div<CardItemProps>`
  width: ${(props) => (props.isGridView ? `${metrics.widths.lg}px` : `${metrics.widths['3xl']}px`)};
  height: ${(props) =>
    props.isGridView ? `${metrics.heights['4xl']}px` : `${metrics.heights.xl}px`};
...
export { CardItemStyled, CardImgWrapperStyled, CardInfoStyled, CardContentStyled, CardTitleStyled };
