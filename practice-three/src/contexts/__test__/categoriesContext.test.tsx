...
import { CategoriesContext, CategoriesProvider, ICategoriesContext } from '../CategoriesContext';
import { categories, categories as mockCategories } from '../../constants/mockData';
...
describe('Test CategoriesContext', () => {
  ...
  it('should provide context values to children', () => {
    ...
    expect(screen.getByTestId('error')).toHaveTextContent(mockContextValue.error);
  });
  ...
  it('should call setSelectedCategory when a category is clicked', () => {
    ...
    render(
      <CategoriesContext.Provider value={mockContextValue as unknown as ICategoriesContext}>
        <button onClick={() => mockContextValue.setSelectedCategory('1')}>Selected Category</button>
      </CategoriesContext.Provider>
    );
  ...
  it('should call handleRemoveSelectedCategory when a category is removed', () => {
    render(
      <CategoriesContext.Provider value={mockContextValue as unknown as ICategoriesContext}>
        <button onClick={() => mockContextValue.handleRemoveSelectedCategory('1')}>
          Remove Category
        </button>
      </CategoriesContext.Provider>
    );
    ...
    fireEvent.click(screen.getByRole('button'));
    expect(mockContextValue.handleRemoveSelectedCategory).toHaveBeenCalledWith('1');
  });

  it('should call getCategoryById when a category is clicked', () => {
    render(
      <CategoriesContext.Provider value={mockContextValue as unknown as ICategoriesContext}>
        <button onClick={() => mockContextValue.getCategoryById('1')}>Remove Category</button>
      </CategoriesContext.Provider>
    );
